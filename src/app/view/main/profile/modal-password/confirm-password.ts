...
export default class ConfirmPasswordView {
  ...
  public configureView(): void {
    this.input?.setAttribute(ListAttributes.ID, ListOfValues.CONFIRM_PASSWORD);
    ...
    this.label?.setAttribute(ListAttributes.FOR, ListOfValues.CONFIRM_PASSWORD);
    this.label?.classList.add(...ListClasses.LABEL_CHANGE_PASSWORD.split(' '));
    if (this.label) {
      this.label.textContent = ListTextContent.CONFIRM_PASSWORD;
    }

    this.eyeImage?.getHTMLElement()?.setAttribute(ListAttributes.SRC, ListPaths.EYE_CLOSE);
    this.eyeImage?.getHTMLElement()?.classList.add(...ListClasses.EYE_IMAGE_3.split(' '));
