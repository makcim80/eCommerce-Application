...
export default class NewPasswordView {
  ...
  public configureView(): void {
    this.input?.setAttribute(ListAttributes.ID, ListOfValues.NEW_PASSWORD);
    ...
    this.label?.setAttribute(ListAttributes.FOR, ListOfValues.NEW_PASSWORD);
    this.label?.classList.add(...ListClasses.LABEL_CHANGE_PASSWORD.split(' '));
    if (this.label) {
      this.label.textContent = ListTextContent.NEW_PASSWORD;
    }

    this.eyeImage?.getHTMLElement()?.setAttribute(ListAttributes.SRC, ListPaths.EYE_CLOSE);
    this.eyeImage?.getHTMLElement()?.classList.add(...ListClasses.EYE_IMAGE_2.split(' '));
