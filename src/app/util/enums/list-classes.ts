export const enum ListClasses {
  ...
  PAGE_BASKET_VIEW = 'justify-center px-3 py-5 flex flex-wrap sm-s:gap-y-2 sm-s:flex-nowrap sm-s:justify-between',
  DIV_IMG_ORDER = 'w-36 h-32',
  IMG_ORDER_BASKET = 'w-full h-full object-cover',
  LIST_OF_ORDER = 'flex flex-col justify-center gap-y-3',
  PRODUCT_LINE = 'flex bg-profile-bg p-3 rounded-2xl justify-between max-w-xl',
  ORDER_NAME_PRODUCT = 'text-xs sm-s:text-xx-s max-ms:text-xs',
  ...
  ORDER_PRICE = 'text-0.65rem text-center sm-s:text-xs lg-l:text-sm',
  BASKET_DELETE = 'w-7 h-7 cursor-pointer absolute right-0 bottom-0 hover:opacity-70',
  TOTAL_CONTENT = 'flex justify-between',
  BASKET_SUMMARY = 'py-5 w-64 m-auto 606px:ml-2 606px:m-0 606px:py-0',
